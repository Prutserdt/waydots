...
- [[#configel][config.el]]
  - [[#save-to-the-system-clipboard][Save to the system clipboard]]
  ...
  - [[#ghostel-ghostty-in-emacs][Ghostel, ghostty in emacs]]
  ...
  - [[#layout-for-hacking-code-scratch-buffer-and-emacs-messages][Layout for hacking code (scratch buffer and emacs messages)]]
...
Ghostel is a emacs package for the ghostty terminal. For now I also keep the vterm-toggle package.
#+BEGIN_SRC emacs-lisp :tangle packages.el :results silent
(package! ghostel)
...
The large language model package gptel.
#+BEGIN_SRC emacs-lisp :tangle packages.el :results silent
(package! gptel)
#+END_SRC

~org-roam~ is a plain-text knowledge management system. It brings some of Roam's more powerful features into the Org-mode ecosystem. ~org-roam-ui~ is a frontend for exploring and interacting with your org-roam notes. Unpinning is necessary for Roam emacs according to de developer of org-roam-ui.
#+BEGIN_SRC emacs-lisp :tangle packages.el :results silent
(unpin! org-roam)
(package! org-roam-ui)
#+END_SRC
...
Lexical binding makes the file run (slightly) faster.
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
;;; -*- lexical-binding: t; -*-
#+END_SRC

Enables =recentf= and configures Emacs to open the most recently used file at startup.  
Uses both =initial-buffer-choice= and =emacs-startup-hook=, with =when-let= safely checking for an available file.
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
(require 'recentf)
(require 'subr-x)
(require 'cl-lib)
...
Everything should be placed in the system clipboard.
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
...
** Custom keybindings
Emacs uses lots of keybindings and Doom Emacs even more on top of it, much of them are Evil mode which is VIM emulation. 

I want to use the =undo-redo= package in Evil mode and therefore add the keybinding =U= here.
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
(after! evil
...
** Right mouse click
The top menu is removed but this does give interesting information. I bound it to rightmouse click to give the menu.
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
(map! :map global-map
...
I changed the background to be inline with my overall theme.... FIXME: needs bettter implementation
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
...
A *superior* font is set here for all the text.
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
...
For more focus the distractions on the screen can be toggled off all together, or separately one by one.
...
This toggle will increase/decrease the width of the text. Not really much to do with distractions actually.
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
...
** Ghostel, ghostty in emacs
A terminal emulator for Emacs powered by libghostty-vt.
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
...
** Jumping around
Jump right forward to the last jump in the jump-list.
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
...
Some preferences that I like: hide the emphasis markup for: /italic/, *bold*, ~code~, _underscore_, =verbatim= and +strikethrough+. Show ▼ for folded sections in org. By default I want all of my org files to show images as default and hide the =#+TITLE= keyword.
#+BEGIN_SRC emacs-lisp :tangle config.el :results silent
...
** Layout for hacking code (scratch buffer and emacs messages)
This code defines a function to quickly toggle between a single buffer and a more complex hacking layout featuring the messages window on the left and a scratch buffer on the right. The function =my-toggle-hacking-layout= facilitates seamless transitions, enhancing your coding efficiency.
#+BEGIN_SRC emacs-lisp :tangle config.el  :results silent
