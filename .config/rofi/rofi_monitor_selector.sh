  ...
  for monitor in $monitors; do
    if [ "$monitor" != "$selected_monitor" ]; then
      ...
      if ! hyprctl keyword monitor "$monitor.disable"; then
        echo "Error: Failed to disable monitor: $monitor"
      fi
    fi
  done

  # Optionally enable the selected monitor if needed
  echo "Enabling monitor: $selected_monitor"
  if ! hyprctl keyword monitor "$selected_monitor.enable"; then
    echo "Error: Failed to enable monitor: $selected_monitor"
  fi
fi
